...
func calculate_miss_chance(in_attacker, in_enemy, in_weapon):
	...
	var dist = vec.length()
	var aim = in_attacker.traits.get_aim()
	var size = pow(1+in_enemy.traits.get_size(),2)
	var factor = pow(in_attacker.traits.get_aim()*in_enemy.traits.get_size(),-0.1)
	var hit_chance = (-sigmoid(0.1*in_weapon.hit_chance_multiplier*(factor*dist - in_weapon.range)) + 1)
	return 1 - hit_chance
